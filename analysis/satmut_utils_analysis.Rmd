...
library(viridis)
library(factoextra)
base_dir<- "/Users/ianhoskins/data_staging/Methods_paper"
...
compare_dir<- "/Users/ianhoskins/data_staging/Methods_paper/variant_call_compare/combined"
...
# Do not apply count filter yet for comparison, so FP differences can be observed
compare_dmstools2_melt_filt_dt<- compare_dmstools2_melt_dt[CAO>0 & wildtype!=ALT]
...
compare_satmut_utils_merge_dmstools_dt[is.na(Truth_CAO), Truth:="F"]
...
# compare_satmut_utils_merge_dmstools_filt_dt[,sum(!is.na(Truth_CAO))] is 281
...
# Enrich2 calls
# Discard the counts for WT and synonymous variants; not any easy format to parse
...
compare_enrich_dt<- rbind(compare_enrich_main_dt[!grepl("_wt", V1)], 
                          compare_enrich_synon_dt[!grepl("_wt|_sy", V1)])
...
compare_satmut_utils_merge_dmstools_enrich_filt_dt<- compare_satmut_utils_merge_dmstools_enrich_dt[!is.na(CAO.z)]
# compare_satmut_utils_merge_dmstools_enrich_filt_dt[,sum(!is.na(Truth_CAO))] confirms all truth variants still present
...
compare_satmut_utils_merge_dmstools_enrich_filt_dt[is.na(Truth_CAO), Truth:="F"]
...
compare_satmut_utils_merge_dmstools_enrich_filt_count_acc_dt<- 
  compare_satmut_utils_merge_dmstools_enrich_filt_dt[!is.na(Truth_CAO),]
...
compare_satmut_utils_merge_dt[is.na(Truth_CAO), Truth:="F"]
compare_satmut_utils_merge_dt[,Truth:=factor(Truth, levels=c("F", "T"))]
compare_satmut_utils_merge_dt[!grepl("False", MATCHES_MUT_SIG), satmut_utils_called:="T"]
compare_satmut_utils_merge_dt[grepl("False", MATCHES_MUT_SIG), satmut_utils_called:="F"]
...
compare_satmut_utils_merge_summ_dt[is.nan(Truth_CAO), Truth:="F"]
...
# Remove satmut_utils calls not in DiMSum calls
...
# Determine satmut_utils-nt performance on tile 6 with -m 2 -q 30 -s NNK, with MUT_SIG filter
...
compare_satmut_utils_merge_dt[CAO.y >= count_thresh & !grepl("False", MATCHES_MUT_SIG), satmut_utils_called:="T"]
compare_satmut_utils_merge_dt[CAO.y < count_thresh | grepl("False", MATCHES_MUT_SIG), satmut_utils_called:="F"]
compare_satmut_utils_merge_dt[,satmut_utils_called:=factor(satmut_utils_called, levels=c("F", "T"))]
...
compare_satmut_utils_merge_summ_dt[MATCHES_MUT_SIG=="False", satmut_utils_called:="F"]
...
compare_satmut_utils_merge_summ_dt[CAO >= count_thresh & MATCHES_MUT_SIG=="True", satmut_utils_called:="T"]
compare_satmut_utils_merge_summ_dt[,satmut_utils_called:=factor(satmut_utils_called, levels=c("F", "T"))]
...
compare_dimsum_aa_change_merge_filt_dt[is.nan(Truth_CAO) | is.na(Truth_CAO), Truth:="F"]
...
## Error modeling with sim on real reads with thousands of CBS variants at variable frequencies
...
roth_ec_dir<- "/Users/ianhoskins/data_staging/Methods_paper/error_correction/roth_cbs"
...
cenik_ec_dir<- "/Users/ianhoskins/data_staging/Methods_paper/error_correction/cenik_cbs"
...
varImpPlot(rf_model_all, type=1)
...
model_filt_props_melt_gg<- ggplot(
  ...
  aes(x=Dataset, y=Count, fill=Filter, label=as.character(Count)))
...
## Primer masking on simulated datasets
...
dedup_dir<- "/Users/ianhoskins/data_staging/Methods_paper/error_correction/cenik_cbs/amp"
...
cbs_02dec2021_both_postprocess_subtract_rf_mut_cast_dt<- dcast(
  ...
  Chemistry+VAR_ID+TYPE+VAR_TYPE+AA_POS_int~Input, value.var="Median_log10_CAF")

cbs_02dec2021_both_postprocess_subtract_rf_mut_cast_dt[,log10_diff:=cDNA-gDNA]

mut_type_gg<- ggplot(
  data=cbs_02dec2021_both_postprocess_subtract_rf_mut_cast_dt, 
  aes(y=log10_diff, x=VAR_TYPE, col=VAR_TYPE))
...
mut_type_gg+
  ...
  theme_cowplot()+
  labs(y="log10 diff", col="Mutation")
...
mut_type_silent_vs_missense_wilcox_res<- 
  ...
  VAR_TYPE%in%c("Silent", "Missense"), wilcox.test(
    log10_diff~VAR_TYPE, .SD, alternative="greater")$p.value, by=.(Chemistry)]
...
cbs_02dec2021_racelike_postprocess_subtract_rf_mut_cast_dt<- dcast(
  ...
  VAR_ID+TYPE+VAR_TYPE+AA_POS_int~Input, value.var="Median_log10_CAF")

cbs_02dec2021_racelike_postprocess_subtract_rf_mut_cast_dt[,log10_diff:=cDNA-gDNA]
...
mut_type_racelike_silent_gg<- ggplot(
  ...
  aes(x=AA_POS_int, y=log10_diff, col=VAR_TYPE))
...
mut_type_racelike_missense_gg+
  ...
  coord_cartesian(xlim=c(0,550), ylim=c(-2,2))+
  theme_cowplot()+
  labs(x="CBS amino acid position", 
       y="log10 freq. diff., cDNA-gDNA")
...
cbs_02dec2021_amplicon_postprocess_subtract_filt_counts_dt<- 
  cbs_02dec2021_both_postprocess_subtract_rf_drop_dt[
    Chemistry=="Amplicon", .N, by=.(VAR_ID)]
...
amplicon_gDNA_med_log10_caf<- cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_dt[
  Input=="gDNA", median(log10_CAF)]

amplicon_cDNA_med_log10_caf<- cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_dt[
...
cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_eset_fit_eb_top_gg<- ggplot(
  ...
  aes(x=log10_CAF, y=reorder(ID_SHORT, AA_POS_int), col=Input))

cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_eset_fit_eb_top_gg+
  geom_vline(xintercept=amplicon_cDNA_med_log10_caf, col=aaas_cols[1], linetype=2)+
  geom_vline(xintercept=amplicon_gDNA_med_log10_caf, col=aaas_cols[2], linetype=2)+
...
amplicon_in_racelike_mat<- copy(cbs_02dec2021_racelike_postprocess_subtract_filt_cast_impute_mat)[,-1]
amplicon_in_racelike_mat<- apply(amplicon_in_racelike_mat, 2, as.numeric)
amplicon_in_racelike_mat_orig<- copy(amplicon_in_racelike_mat)
...
racelike_lod<- max(apply(amplicon_in_racelike_mat_orig, 2, min, na.rm=T))

amplicon_in_racelike_mat_orig_nas<- apply(
  amplicon_in_racelike_mat_orig, 1, function(x) sum(is.na(x)))
...
amplicon_in_racelike_dt[,VAR_ID:=cbs_02dec2021_racelike_postprocess_subtract_filt_cast_impute_mat[,1]]
amplicon_in_racelike_dt[,N_NAs:=amplicon_in_racelike_mat_orig_nas]
...
amplicon_in_racelike_sig_melt_dt<- melt(
  ...
  variable.name="Sample", value.name="log10_CAF")
...
amplicon_in_racelike_sig_merge_dt<- rbind(
  ...
  cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_dt[
    VAR_ID%in%cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_top_varids,
    .(Chemistry, VAR_ID, Sample, log10_CAF)], fill=TRUE)
...
amplicon_in_racelike_sig_merge_cast_dt<- dcast(
  amplicon_in_racelike_sig_merge_dt[
    ,.(Median_log10_CAF=median(log10_CAF)),
    by=.(Chemistry, Input, VAR_ID)],
  VAR_ID~Chemistry+Input, value.var="Median_log10_CAF")

sig_annots_dt<- unique(cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_sig_dt[
...
amplicon_in_racelike_sig_merge_cast_annot_dt[abs(delta_delta)>0.7, Label:=ID_SHORT]
...
amplicon_in_racelike_gg<- ggplot(
  data=amplicon_in_racelike_sig_merge_cast_annot_dt[!is.na(delta_delta)],
  aes(x=Amplicon_diff, y=RACElike_diff, label=Label))

amplicon_in_racelike_gg+
  geom_abline(slope=1, col="grey", linetype=2)+
  geom_hline(yintercept=0, col="grey", linetype=3)+
  geom_vline(xintercept=0, col="grey", linetype=3)+
  geom_point(aes(col=N_NAs), size=2)+
  geom_text_repel(box.padding=1)+
  labs(x="Amplicon log10 freq. diff.", 
       y="RACE-like log10 freq. diff.", 
       col="Number NAs (of 6)")+
  theme_cowplot()

amplicon_in_racelike_sig_merge_cast_annot_dt[
  !(is.na(Amplicon_diff)|is.na(RACElike_diff)),
  cor(Amplicon_diff, RACElike_diff, method="spearman")]
...
amplicon_summ_cast_dt<- dcast(
  ...
  AA_POS+REF_AA+ALT_AA~Input, value.var="Mean_log10_CAF")

#na_val<- (-6.5)
#amplicon_summ_cast_dt[is.na(cDNA), cDNA:=na_val]
#amplicon_summ_cast_dt[is.na(gDNA), gDNA:=na_val]
...
#ve_heatmap_col_palette<- colorRampPalette(brewer.pal(11, "PRGn"))(100)
#scale_fill_gradientn(colors=ve_heatmap_col_palette)+
#geom_vline(xintercept=c(52,65,119), col="grey", linetype=2)+
...
The list of mutations we will validate are:
Down:
K39V- uncharac.
R58I
H65R
...
E128I
T135C- uncharac.
V255Y:5324:GTG:TAT (RACE-like, not sig.)

Up:
R45P
C109G
C109T
V448G:5904:TG:GT (RACE-like, not sig.)

## Compare domains

Determine if their are global position-specific effects.
...
amplicon_ve_by_pos_gg+
  ...
  labs(x="Amino acid position",
       y="Min cDNA-gDNA log10 freq per AA")+
  theme_cowplot()
...
roth_fc_dir<- "/Users/ianhoskins/data_staging/Methods_paper/CBS_02DEC2021/Genome_Medicine_2020_suppl"
...
## tRNAseq correlation with limma differential variants
...
Further investigate data from the following papers:
Quantitative tRNA-sequencing uncovers metazoan tissue-specific tRNA regulation (QuantM-tRNAseq)
https://www.nature.com/articles/s41467-020-17879-x

Rapid and Scalable Profiling of Nascent RNA with fastGRO
https://www.cell.com/cell-reports/pdf/S2211-1247(20)31362-0.pdf
...
cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all<- topTable(cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb, coef=ncol(cbs_02dec2021_amplicon_postprocess_subtract_filt_design), number=nrow(cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb))

cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_dt<- as.data.table(
  cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all)

cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_dt[
  ,VAR_ID:=rownames(cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all)]

cbs_02dec2021_amplicon_postprocess_subtract_filt_uniq_dt<- unique(cbs_02dec2021_amplicon_postprocess_subtract_filt_dt[,.(VAR_ID, REF, ALT, REF_CODON, ALT_CODON, AA_CHANGE, AA_POS)])
...
cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
  Direction!="N.S." & (CODON_OPTI_diff>0.05 | CODON_OPTI_diff<(-.13)), Label:=gsub("p.", "", AA_CHANGE)]
...
codon_optimality_merge_gg<- ggplot(
  ...
  aes(x=Direction, y=CODON_OPTI_diff, label=Label))
...
codon_optimality_merge_gg+
  ...
  geom_jitter(data=cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
    Direction!="N.S."], aes(x=Direction, y=CODON_OPTI_diff, col=adj.P.Val), position=jitter_pos)+
  geom_text_repel(data=cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
    Direction!="N.S."], aes(x=Direction, y=CODON_OPTI_diff), box.padding=0.7, position=jitter_pos)+
...
# Remove variants that do not have either
# 1 N.S. and 1 RNA up/down

codon_opti_comp_aa_changes_drop<- c("p.A38C", "p.E144L", "p.P138R")
codon_opti_inconsistent_aa_changes<- c("p.H67T", "p.A130S", "p.P41T", "p.S123S", "p.C109G")
codon_opti_inconclusive_aa_changes<- c("p.P59L", "p.P138G", "p.E37I", "p.S50S", "p.A38T", "p.R121L", "p.R125L", "p.H66R", "p.H67G", "p.N113G")
...
csc_diff_cast_dt<- dcast(data=cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_opti_comp_final_dt, AA_CHANGE~VAR_ID, value.var="CODON_OPTI_diff")
...
codon_opti_comp_aa_changes_gg<- ggplot(
  data=cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_opti_comp_final_dt[!AA_CHANGE%in%csc_diff_nonsig_aa_changes],
...
codon_opti_comp_aa_changes_gg+
  ...
  geom_point(aes(col=Direction), size=2)+
  labs(y="Amino acid", x="Alt. - Ref. CSC", col="Direction")+
  theme_cowplot()+
  theme(legend.position="bottom")

#pbinom(9,13,0.5,lower.tail=FALSE)
...
tRNA_dir<- "/Users/ianhoskins/data_staging/Methods_paper/CBS_02DEC2021/tRNA"
...
cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
  ,REF_CODON_hydro_log2_sum:=hydro_tRNAseq_raw_agg_wobble_summ_dt[
    match(cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[,REF_CODON], Codon), log2_sum_count]]

cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
  ,ALT_CODON_hydro_log2_sum:=hydro_tRNAseq_raw_agg_wobble_summ_dt[
    match(cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[,ALT_CODON], Codon), log2_sum_count]]
...
mim_trnaseq_cor_gg<- ggplot(
  ...
  aes(x=mim_log2_sum_diff, y=logFC, col=adj.P.Val, label=Label))
...
cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
  !is.na(mim_log2_sum_diff) & adj.P.Val < codon_opti_fdr_thresh & logFC < 0, 
  cor(mim_log2_sum_diff, logFC, method="spearman")]


cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[,Label:=NA]

cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
  logFC < (-1.8), Label:=gsub("p.", "", AA_CHANGE)]


hydroseq_dt<- cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
    !is.na(hydro_log2_sum_diff) & adj.P.Val < codon_opti_fdr_thresh]
    
hydro_trnaseq_cor_gg<- ggplot(
  data=hydroseq_dt[logFC<0], 
  aes(x=hydro_log2_sum_diff, y=logFC, col=adj.P.Val, label=Label))
...
cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
  !is.na(hydro_log2_sum_diff) & adj.P.Val < codon_opti_fdr_thresh & logFC < 0, 
...
Determine if variants that increase/decrease abundance are detected in yeast functional complementation screen.

```{r Correlate logFC of cDNA:gDNA to fitness of variants in yeast functional complementation}
...
cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all<- topTable(
  cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb, coef=ncol(
      cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_design), 
...
cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_dt<- as.data.table(
  cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all)

cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_dt[
  ,VAR_ID:=rownames(cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all)]

cbs_02dec2021_racelike_postprocess_subtract_filt_uniq_dt<- unique(cbs_02dec2021_racelike_postprocess_subtract_filt_dt[
  ,.(VAR_ID, REF, ALT, REF_CODON, ALT_CODON, AA_CHANGE, AA_POS)])

cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt<- 
  merge(cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_dt,
        cbs_02dec2021_racelike_postprocess_subtract_filt_uniq_dt, by="VAR_ID")

amplicon_racelike_merge_cols<- c("VAR_ID", "REF", "ALT", "REF_CODON", "ALT_CODON", "AA_CHANGE", "AA_POS", "logFC", "AveExpr", "adj.P.Val")

cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_dt<- rbind(
  cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
    ,..amplicon_racelike_merge_cols],
  cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt[
    ,..amplicon_racelike_merge_cols])

cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_dt[
  ,Chemistry:=as.factor(rep(
    c("Amplicon", "RACE-like"),
    times=c(nrow(cbs_02dec2021_amplicon_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt),
            nrow(cbs_02dec2021_racelike_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_all_merge_dt))))]

# Annotate Hydro-tRNAseq counts
cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_dt[
  ,REF_CODON_hydro_log2_sum:=hydro_tRNAseq_raw_agg_wobble_summ_dt[
    match(cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_dt[,REF_CODON], Codon), log2_sum_count]]

cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_dt[
  ,ALT_CODON_hydro_log2_sum:=hydro_tRNAseq_raw_agg_wobble_summ_dt[
    match(cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_dt[,ALT_CODON], Codon), log2_sum_count]]
...
cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_dt[
  ,REF_CODON_OPTI:=codon_optimality_wu_2019_dt[
    match(cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_dt[,REF_CODON], codon), `293T_ORFome`]]

cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_dt[
  ,ALT_CODON_OPTI:=codon_optimality_wu_2019_dt[
    match(cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_dt[,ALT_CODON], codon), `293T_ORFome`]]
...
cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_yfc_dt<- merge(
  ...
  roth_yeast_fc_st4_dt, by="AA_CHANGE")
...
cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_yfc_examp_dt<-
  cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_yfc_dt[
    , .(Chemistry, VAR_ID, REF, ALT, REF_CODON, ALT_CODON, AA_CHANGE, logFC, AveExpr, adj.P.Val, Max_score, LLR, flags, hydro_log2_sum_diff, mim_log2_sum_diff, CODON_OPTI_diff)]
...
write.table(
  cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_yfc_examp_dt,
  paste(cbs_02dec2021_dir, "cbs_02dec2021_both_postprocess_subtract_rf_yfc_fix.txt", 
        sep="/"), sep="\t", quote=FALSE, row.names=FALSE)

# pbinom(61,108,0.5,lower.tail=FALSE)
...
yfc_final_varids<- c("pDEST_HC_Rec_Bxb_v2_CBS_WT:4673:GC:TG", "pDEST_HC_Rec_Bxb_v2_CBS_WT:4673:G:A", 
                     ...
                     "pDEST_HC_Rec_Bxb_v2_CBS_WT:4760:CAC:ACG", "pDEST_HC_Rec_Bxb_v2_CBS_WT:4676:AAG:GTT", 
                     "pDEST_HC_Rec_Bxb_v2_CBS_WT:4973:CCC:TGT", "pDEST_HC_Rec_Bxb_v2_CBS_WT:4733:CGG:ATT")

cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_yfc_examp_final_dt<- 
  cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_yfc_examp_dt[
    VAR_ID%in%c(yfc_final_varids), .(AA_CHANGE, REF_CODON, ALT_CODON, logFC, Max_score)]
...
write.table(
  cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_yfc_examp_final_dt,
  paste(cbs_02dec2021_dir, "cbs_02dec2021_both_postprocess_subtract_rf_yfc_examp_fix.txt", 
        sep="/"), sep="\t", quote=FALSE, row.names=FALSE)
...
#cbs_02dec2021_both_postprocess_subtract_filt_logCPM_eb_top50_yfc_merge_val_dt<- merge(cbs_02dec2021_both_postprocess_subtract_filt_logCPM_eb_top50_yfc_merge_dt, roth_yeast_fc_st1_dt, by="AA_CHANGE")
...
cbs_02dec2021_both_postprocess_subtract_rf_yfc_filt_cast_merge_val_dt<-
  merge(cbs_02dec2021_both_postprocess_subtract_filt_limma_freq_cast_eset_fit_eb_merge_yfc_dt,
        roth_yeast_fc_st1_dt, by="AA_POS")

cbs_02dec2021_both_postprocess_subtract_rf_yfc_filt_cast_merge_val_final_dt<-
  cbs_02dec2021_both_postprocess_subtract_rf_yfc_filt_cast_merge_val_dt[
  ,.(Chemistry, VAR_ID, REF_CODON, ALT_CODON, 
     logFC, adj.P.Val, mim_log2_sum_diff, hydro_log2_sum_diff, 
     AA_CHANGE.x, AA_CHANGE.y, Label, flags,
     PROVEAN, `PolyPhen-2`, CADD, MAF, LLR, `VE map fitness\n(low vitamin B6)`, 
     `VE map fitness\n(high vitamin B6)`)]

write.table(
  cbs_02dec2021_both_postprocess_subtract_rf_yfc_filt_cast_merge_val_final_dt,
  paste(cbs_02dec2021_dir, "cbs_02dec2021_both_postprocess_subtract_filt_logCPM_eb_top50_yfc_merge_val_final_fix.txt", 
        sep="/"), sep="\t", quote=FALSE, row.names=FALSE)
...
## RT-qPCR validation of individual CBS mutants

```{r rt-qpcr analysis}

rtqpcr_dir<- "/Users/ianhoskins/iCloud\ Drive\ \(Archive\)/Documents/Cenik_lab/Experiments/CBS_design/CBS_LLP_RNA_expr_validation_mutants"

rtqpcr_res_dt<- fread(paste(cbs_02dec2021_dir, "CBS_LLP_RNA_expr_validation_mutants_global_analysis.txt", sep="/"), header=T)

rtqpcr_res_gg<- ggplot(
  data=rtqpcr_res_dt[Trial!="Trial_2"], 
  aes(x=Mutant, y=CBS_EEF2_FC_mean, ymax=CBS_EEF2_FC_mean+CBS_FC_SD, 
      ymin=CBS_EEF2_FC_mean-CBS_FC_SD, col=Trial))
...
rtqpcr_res_gg+
  ...
  theme_cowplot()+
  labs(x="Mutant", y="Fold change, CBS to EEF2", col="Trial")+
